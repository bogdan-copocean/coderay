"""
Vector store using LanceDB (embedded, directory-based).

Index lives in a gitignored directory; portable and plug-and-play. Columnar
storage: data is stored by column, not by row. Vector search reads mostly the
vector column; filters (path, language) read only those columns. Each insert
adds new fragments; old versions and deleted rows accumulate until maintenance.
"""
...
class Store:
    ...
    def __init__(self, db_path: str | Path, dimensions: int = DEFAULT_DIMENSIONS):
        p = Path(db_path)
        if p.suffix and p.name != "":
            self.db_path = p.parent
        else:
            self.db_path = p
    ...
    def search(
        ...
        """
        Nearest-neighbor search with optional native BM25 hybrid scoring.

        When query_text is provided, uses LanceDB's native hybrid search
        (vector + FTS with RRF reranking). Otherwise falls back to pure
        vector search.
        """
        if not self._table_exists():
            return []
        ...
        for r in rows:
            row = dict(r)
            score = row.pop(
                "_distance", row.pop("_relevance_score", row.pop("distance", 0.0))
            )
