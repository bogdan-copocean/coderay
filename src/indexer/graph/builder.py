"""Build the code graph from source files and persist to JSON."""
...
def build_and_save_graph(
    ...
    If ``changed_paths`` is provided and a previous graph exists on disk,
    only the changed files are re-parsed and merged into the existing graph
    (O(changed) instead of O(all)).  Otherwise a full rebuild is performed.
    """
