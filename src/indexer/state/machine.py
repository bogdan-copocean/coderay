"""
Index metadata and file-hash state (meta.json, file_hashes.json).

Manages state machine for indexer runs: in_progress, done, errored, incomplete.
"""
...
class StateMachine:
    """
    Loads and saves index meta and file hashes.

    Use property assignment for state: ``self.current_state = value``,
    ``self.file_hashes = new_dict``.
    """
    ...
    def set_incomplete(self) -> None:
        """
        Set state to incomplete when a previous run did not finish.

        Call on startup before a new run to mark stale in_progress as incomplete.
        """
    ...
    def start(self, branch: str | None, last_commit: str | None) -> None:
        """
        Start a new run: set state to in_progress and save meta.

        Args:
            branch: Current branch name.
            last_commit: Current HEAD commit hash.
        """
        now = time.time()
        self._current_state = IndexMeta(
    ...
    def save_progress(
        ...
        """
        Update meta with progress for resume.

        Args:
            full_rel_paths: Full list of paths being processed (not just current batch).
            processed_count: Number of paths processed so far.
        """
        if self._current_state is None or not self.is_in_progress:
            return
        run = CurrentRun(
    ...
    def finish(
        ...
        """
        Mark run as done: save file hashes, set state to done, save meta.

        Args:
            last_commit: HEAD commit at finish (default: keep from current state).
            branch: Branch name at finish (default: keep from current state).
        """
