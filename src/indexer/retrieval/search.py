"""
Retrieval: semantic search and graph/code retrieval from the index.
Single entry point for querying chunks and call/import graph.
"""
...
from indexer.retrieval.boosting import StructuralBooster
from indexer.state.machine import IndexMeta
from indexer.state.version import check_index_version
...
class Retrieval:
    """
    Query the index: semantic search over chunks and load call/import graph.
    Use search() for semantic search; load_graph(), list_chunks(),
    chunks_by_path() for inspection.
    """
    ...
    def search(
        ...
        *,
        ...
        """
        Semantic search over the index. Returns list of dicts: path, start_line,
        end_line, symbol, language, content, score.

        Uses LanceDB native hybrid search (BM25 + RRF) when available,
        then applies structural boosting for final ranking.
        """
