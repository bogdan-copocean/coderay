"""
CLI: index build [--full], index update, index search "query" [--top-k].
Colored output; timing for index/search.
"""
...
from indexer.core.lock import acquire_indexer_lock
...
from indexer.storage.lancedb import index_exists
...
def build(ctx: click.Context, full: bool, repo: Path) -> None:
    ...
    Uses a file lock so only one build/update runs at a time.
    """
...
def update(ctx: click.Context, repo: Path) -> None:
    ...
    if not indexer.index_exists():
        click.echo(_color("No index found. Run 'index build' first.", YELLOW))
        ctx.exit(1)
...
def search_cmd(
    ...
    if current_state is None:
        click.echo(_color("No index state. Run 'index build' first.", YELLOW))
        ctx.exit(1)
    ...
    for i, r in enumerate(results, 1):
        ...
        click.echo("")
        click.echo(
            _color(f"  {i}. {path}:{start}-{end} ({symbol})  score={score:.4f}", GREEN)
        )
...
def maintain(ctx: click.Context, repo: Path) -> None:
    ...
    Cleans up old versions and merges fragments. Also runs
    automatically after build and update.
    """
    index_dir = ctx.obj["index_dir"]
    if not index_exists(index_dir):
        click.echo(_color("No index found. Run 'index build' first.", YELLOW))
...
def graph_cmd(
    ...
    if not edges:
        click.echo(
            _color(
                "No graph data. Run 'index build' or 'index update' to build it.",
                YELLOW,
            )
        )
        ctx.exit(0)
