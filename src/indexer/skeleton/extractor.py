"""Extract file skeletons: signatures, docstrings, imports -- no bodies.

Produces a compact summary of a file's API surface that is highly
token-efficient for LLMs. Function/method bodies are replaced with ``...``.
"""
...
def extract_skeleton(
    ...
    """Extract the skeleton of a source file.

    Returns a string containing imports, class/function signatures with
    docstrings, and top-level assignments. Bodies are replaced with ``...``.
    """
    path_str = str(path)
...
def _get_docstring(node, source_bytes: bytes) -> str | None:
    """Extract docstring from the first expression_statement child if it's a string."""
