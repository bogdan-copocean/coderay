...
from coderay.retrieval.models import Relevance, SearchRequestDTO, SearchResult
...
class Retrieval:
    ...
    def search(
        ...
        with timed_phase("embed"):
            query_vectors = self._embedder.embed([request.query], task=EmbedTask.QUERY)
        ...
        with timed_phase("vector_search"):
            raw_results = store.search(
                ...
                # Overfetch so deduplication doesn't leave the user short.
                # _deduplicate_by_containment drops enclosing spans; fetching
                # 2x absorbs those losses before we trim back below.
