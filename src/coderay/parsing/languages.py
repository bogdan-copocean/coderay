...
@dataclass(frozen=True)
class GraphConfig:
    """Graph extraction policies (not CST node types)."""
...
class PythonConfig:
    ...
    graph: GraphConfig = field(default_factory=lambda: _PYTHON_GRAPH)
...
_JS_TS_CST_DISPATCH = CstDispatchConfig(
    ...
    typed_param_types=("typed_parameter", "required_parameter", "optional_parameter"),
)

_JS_TS_GRAPH = GraphConfig(
    self_prefix="this.",
    super_prefixes=("super().", "super."),
)
...
class JavaScriptConfig:
    ...
    graph: GraphConfig = field(default_factory=lambda: _JS_TS_GRAPH)
...
LANGUAGE_REGISTRY: dict[str, LanguageConfigProtocol] = {
    "python": PythonConfig(),  # type: ignore[dict-item]
    "javascript": JavaScriptConfig(),  # type: ignore[dict-item]
    "typescript": TypeScriptConfig(),  # type: ignore[dict-item]
}
