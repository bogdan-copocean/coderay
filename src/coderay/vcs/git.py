...
class WorkspaceGit:
    """Git discovery and incremental changes across configured index roots."""

    def __init__(self, workspace: object) -> None:
        """Initialize with a resolved workspace from config."""
    ...
    def discover_files(
        ...
        """Return sorted (logical_key, absolute_path) for all indexed files."""
        if extensions is None:
            from coderay.parsing.languages import get_supported_extensions
        ...
        for root in self._ws.roots:
            try:
                g = Git(root.repo_root)
                for p in g.discover_files(extensions=extensions):
                    if root.contains_path(p):
                        out.append((self._ws.logical_key(root, p), p))
            except Exception:
                logger.warning(
                    "Cannot discover files for checkout '%s' at %s; "
                    "its indexed files will be deindexed on next build",
                    root.alias,
                    root.repo_root,
                )
    ...
    def get_files_to_index(
        self,
        root_commits: dict[str, str | None],
    ) -> tuple[list[tuple[str, Path]], list[str]]:
        """Paths to add and logical keys to remove since stored commits per root."""
        to_add: list[tuple[str, Path]] = []
        to_remove: list[str] = []
        for root in self._ws.roots:
            last = root_commits.get(root.alias)
            g = Git(root.repo_root)
            add_paths, remove_rels = g.get_files_to_index(last_commit=last)
            for p in add_paths:
                if root.contains_path(p):
                    to_add.append((self._ws.logical_key(root, p), p))
            for rp in remove_rels:
                rp_posix = rp.replace("\\", "/")
                abs_p = (root.repo_root / rp_posix).resolve()
                if root.contains_path(abs_p):
                    to_remove.append(f"{root.alias}/{rp_posix}")
        merged: dict[str, Path] = {}
        for k, p in to_add:
            merged[k] = p
        to_add = sorted(merged.items(), key=lambda x: x[0])
        to_remove = sorted(set(to_remove))
        return to_add, to_remove

    def head_commits(self) -> dict[str, str | None]:
        """Return current HEAD commit hash per checkout alias."""
        out: dict[str, str | None] = {}
        for root in self._ws.roots:
            out[root.alias] = Git(root.repo_root).get_head_commit()
        return out

    def current_branches(self) -> dict[str, str | None]:
        """Return current branch name per checkout alias."""
        out: dict[str, str | None] = {}
        for root in self._ws.roots:
            out[root.alias] = Git(root.repo_root).get_current_branch()
        return out
