...
class JsTsImportProcessor:
    ...
    def handle(self, node: TSNode, *, scope_stack: list[str]) -> str | None:
        ...
        if raw_source.startswith(("./", "../")):
            mod_path = resolve_relative_import(self._session.module_id, raw_source)
        else:
        ...
        imported = self._collect_imported(import_clause, mod_path)
        if not imported:
            return
    ...
    def _emit_edges(
        ...
        for original, local in imported:
            ...
            append_import_edge(self._session, mid, edge_target)
