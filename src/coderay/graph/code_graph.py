...
class CodeGraph:
    ...
    def get_impact_radius(self, symbol: str, depth: int = 2) -> ImpactResult:
        ...
        if resolved is None:
            return self._ambiguous_or_not_found_result(symbol)
        ...
        raw_nodes: list[GraphNode] = [
            n
            for nid in visited
            if (n := self.get_node(nid)) is not None and nid != own_module
        ]
    ...
    def _ambiguous_or_not_found_result(self, symbol: str) -> ImpactResult:
        """Disambiguation hint for multiple matches, else not-found."""
        sym_candidates = self._symbol_index.get(symbol, set())
        qual_candidates = self._qualified_index.get(symbol, set())
